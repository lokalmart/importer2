...
class LokalmartImporter {
  ...
  isDryId(value) { return typeof value === 'string' && value.startsWith('__dryrun__:'); }
  ...
  domainHasDryId(domain) {
    ...
    return domain.some((item) => Array.isArray(item) ? this.domainHasDryId(item) : this.isDryId(item));
  }
  ...
  async ensureXmlId(xmlid, model, resId, sheet) {
    ...
    if (rows.length) { this.cacheXmlId(xmlid, rows[0].model, rows[0].res_id); return; }
  ...
  async findByXmlIdOrDomain(xmlid, model, domain, fields = ['id']) {
    ...
    try {
      ...
      if (this.dryRun) { this.log.warn('DRY_RUN_LOOKUP', `Lookup ${model} dilewati karena Odoo menolak domain. Ini normal jika field x_* belum benar-benar dibuat saat dry-run.`, { error: e.message, domain }); return null; }
      throw e;
    }
  }
  ...
  async processSelections() {
    ...
    for (const [i, row] of rows.entries()) {
      ...
      if (this.dryRun && this.isDryId(fieldId)) { this.log.info(sheet, `[dry-run] create selection ${value}`, values); continue; }
  ...
  async processProjectTags() {
    ...
    for (const row of rows) { const xmlid = row.external_id || row.id; const name = row.name; if (isBlank(name)) continue; const found = await this.findByXmlIdOrDomain(xmlid, 'project.tags', [['name', '=', name]], ['id', 'name']); if (found) { this.log.ok(sheet, `Tag sudah ada: ${name}`); await this.ensureXmlId(xmlid, 'project.tags', found.id, sheet); continue; } if (this.dryRun) { this.dryRecord(xmlid, 'project.tags', sheet, `[dry-run] create tag ${name}`); continue; } const id = await this.odoo.create('project.tags', { name }); this.log.ok(sheet, `Tag dibuat: ${name}`, { id }); await this.ensureXmlId(xmlid, 'project.tags', id, sheet); }
  }

/**
 * Lokalmart Universal Odoo RPC Proxy V7
 * Tidak menyimpan credential. Semua credential dikirim hanya untuk request berjalan.
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    const { baseUrl, db, login, password, uid: providedUid, model, method, args = [], kwargs = {} } = body;
    ...
    if (!uid) return res.status(401).json({ ok: false, error: "Authentication failed. Check database, login, and password/API key." });

    if (!model || !method) return res.status(200).json({ ok: true, uid, result: true });
